...
const configPath = resolve(process.cwd(), process.env.NAMCHE_PROXY_CONFIG ?? 'api-proxy/routes.config.json');
...
app.get('/healthz', (c) => {
  return c.json({ ok: true, service: 'api-proxy', configPath, agents: Object.keys(agents) });
});
...
app.post('/webhooks/:agent/:source', async (c) => {
  ...
  try {
    ...
    console.log(`[api-proxy] agent=${agentId} source=${source} status=${upstream.status} bytes=${body.byteLength}`);
    ...
    console.error(`[api-proxy] agent=${agentId} source=${source} error=${message}`);
...
serve({ fetch: app.fetch, hostname: listenHost, port: listenPort }, () => {
  console.log(`[api-proxy] listening on ${listenHost}:${listenPort}`);
  console.log(`[api-proxy] config ${configPath}`);
});
